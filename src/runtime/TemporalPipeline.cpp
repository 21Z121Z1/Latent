...
std::uint64_t temporalWorkingSetBound(imaging::Extent extent, std::size_t members,
    ...
    constexpr std::uint64_t tileBytes = 2U*sizeof(reference::MotionTile) +
        sizeof(reference::RegistrationEvidence) + sizeof(imaging::RegionSupport);
    constexpr std::uint64_t frameBytes = sizeof(TemporalFrameTrace) +
        sizeof(imaging::FrameContribution) + sizeof(imaging::FrameId);
    if (n > maximum/bytesPerPixel || tiles > (maximum-frameBytes)/tileBytes ||
        members > maximum/(tiles*tileBytes+frameBytes))
        throw std::invalid_argument("temporal working-set arithmetic overflow");
    const auto imageBytes = n*bytesPerPixel, traceBytes = (tiles*tileBytes+frameBytes)*members;
...
TemporalResult executeTemporalPlan(const TemporalExecutionPlan& plan, const HostRawBindings& bindings, const TemporalExecutionControl& control) {
    ...
    trace.frames.reserve(burst.members.size());
    auto lineage = std::make_shared<imaging::ImageLineage>();
    lineage->contributions.reserve(burst.members.size());
